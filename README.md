# 👋 Hi, I'm Raj Kumar

## 🚀 About Me

I'm a software engineer focused on building **developer tools**, **distributed systems**, and **AI-powered applications**.

I enjoy solving challenging engineering problems—from designing high-performance backend frameworks and package managers to building scalable AI platforms, cloud-native services, and modern full-stack applications.

Most of my work revolves around **Go**, **Python**, **C++**, and **TypeScript**, with a strong interest in system design, performance optimization, and developer experience. I actively contribute to open source and enjoy exploring low-level systems, reproducible builds, and emerging AI technologies.

My interests include:
* 🌐 SDET
* 🤖 AI/ML & LLMs
* ☁️ Cloud & DevOps

## 🎯 Currently Building & Exploring

### 🔨 Building
- 📦 CPM — A reproducible package manager for C/C++
- ⚡ FlashORM — High-performance multi-database ORM
- 🚀 AstraAPI — FastAPI-compatible backend framework in C++

### 📚 Exploring
- 🤖 AI Agents & Autonomous Systems
- 🦀 Systems Programming with Rust
- 🖥️ GPU Computing & AI Infrastructure
- ☁️ Large-scale Distributed Systems

Always learning, building, and shipping new ideas.


---

## 💬 Connect With Me

[![Portfolio](https://img.shields.io/badge/Portfolio-000000?style=for-the-badge&logo=vercel&logoColor=white)](https://ranadolui.me)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/rana-dolui-89357728a/)
[![Email](https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:ranadolui718@gmail.com)



# 🚀 Tech Stack

## 🧠 Languages
![Languages](https://go-skill-icons.vercel.app/api/icons?i=go,python,ts,js,cpp,c,rust,bash&perline=12)

## ⚛️ Backend & AI
![Backend](https://go-skill-icons.vercel.app/api/icons?i=fastapi,fiber,expressjs,nodejs,graphql,grpc,socketio,langchain,pytorch,huggingface&perline=12)

## 🎨 Frontend & Mobile
![Frontend](https://go-skill-icons.vercel.app/api/icons?i=react,nextjs,svelte,solidjs,tanstack,reactnative,expo,tailwind,zustand,redux&perline=12)

## 🗄️ Databases
![Databases](https://go-skill-icons.vercel.app/api/icons?i=postgresql,mysql,sqlite,supabase,prisma,cassandra,mongodb,redis,pinecone&perline=12)

## ☁️ DevOps & Infrastructure
![DevOps](https://go-skill-icons.vercel.app/api/icons?i=docker,kubernetes,aws,gcp,githubactions,nginx,terraform,kafka,rabbitmq,linux&perline=12)

## 🛠️ Tools
![Tools](https://go-skill-icons.vercel.app/api/icons?i=nixos,git,github,postman,vite,pnpm,figma,androidstudio&perline=12)

## 📊 GitHub Stats

![Rana's GitHub stats](https://github-readme-stats.vercel.app/api?username=Rana718&show_icons=true&theme=radical&count_private=true)

<a href="http://www.github.com/Rana718"><img src="https://github-readme-streak-stats.herokuapp.com/?user=Rana718&stroke=FF6F61&background=1E1E2E&ring=FF6F61&fire=FF6F61&currStreakNum=FF6F61&currStreakLabel=FF6F61&sideNums=FF6F61&sideLabels=FF6F61&dates=FF6F61&hide_border=true" alt="Rana718's GitHub Streak" /></a>

- GitHub contributions!
![snake gif](https://github.com/Rana718/Rana718/blob/output/github-snake-dark.svg)

![Profile Views](https://komarev.com/ghpvc/?username=Rana718&color=green)


Let's connect and create something amazing together! -->

---





